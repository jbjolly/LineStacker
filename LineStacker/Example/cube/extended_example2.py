...
for (i, im) in enumerate(rebinnedImageNames):
    ia.open(im)
    vv=ia.summary()
    print ''    
    print i
...
freq0int=info['refpix'][-1]
freq0=info['refval'][-1]
freqIncr=info['incr'][-1]
if freq0int==0:
    freqs=np.linspace(freq0, freq0+freqIncr*len(integratedImage), len(integratedImage))
else:
    freq0=freq0-freq0int*freqIncr    
    freqs=np.linspace(freq0, freq0+freqIncr*len(integratedImage), len(integratedImage))

#self.lineWidth=self.velocity*(self.fEm/(1+self.z))*1./c #in Hz
c=299792.458#in km
velocity=c*freqIncr/(freq0)
startVel=-velocity*len(integratedImage)/2.
endVel=startVel+velocity*len(integratedImage)
allVel=np.linspace(startVel, endVel, len(integratedImage))
...
ax=fig.add_subplot(111)
#ax.step(range(len(integratedImage)), integratedImage, 'k', label='stack', where='mid')
ax.step(freqs, integratedImage, 'k', label='stack', where='mid')
ax.plot(freqs, fited, 'g', label='Single Gaussian fit')
ax.plot(freqs, doubleFited[0],'r--', label='Double Gaussian Fit, all')
ax.plot(freqs, doubleFited[1],'m', label='Double Gaussian Fit, 1st component')
ax.plot(freqs, doubleFited[2],'c', label='Double Gaussian Fit, 2nd component')
ax.set_title('rebin')
ax.legend()
fig.show()
